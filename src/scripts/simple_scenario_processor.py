...
class SimpleScenarioProcessor:
    ...
    def parse_excel_file(self, excel_path: str) -> List[CausalScenario]:
        ...
        for idx, row in df.iterrows():
            if pd.isna(row.get('Scenario', '')) or str(row.get('Scenario', '')).strip() == '':
                continue
...
You must provide your reasoning step by step, then give a numerical assessment."""
    ...
    def generate_prompt_variations(self, scenario: CausalScenario) -> List[PromptVariation]:
        ...
        if scenario.original_question:
            ...
            base_question = "What is the degree of causal relationship in this scenario?"
    ...
    def save_to_json(self, output_path: str):
        ...
        data = {
            ...
            "metadata": {
                ...
                "variations_per_scenario": len(self.prompt_variations) // len(self.scenarios) if self.scenarios else 0,
                "variation_types": list(set(v.variation_type for v in self.prompt_variations))
            }
        }
