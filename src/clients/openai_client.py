...
class OpenAIClient:
    ...
    def __init__(self, api_key: Optional[str] = None, cache_enabled: bool = True):
        ...
        if not self.api_key:
            raise ValueError("OPENAI_API_KEY not found. Provide it as parameter or set in .env file")
    ...
    def _generate_cache_key(self, messages: List[Dict[str, Any]], model: str, **kwargs) -> str:
        ...
        for attempt in range(self.retry_attempts):
            try:
                async with session.post(url, json=payload) as response:
                    ...
                    if response.status == 200:
                        ...
                        if attempt < self.retry_attempts - 1:
                            ...
                            wait_time = int(retry_after) if retry_after else self.retry_delay * (2 ** attempt)
                ...
                if attempt < self.retry_attempts - 1:
                    ...
                    print(f"Connection error (attempt {attempt + 1}/{self.retry_attempts}): {str(e)}")
...
async def process_batch(
    ...
    async def process_single_request(request: Dict[str, Any]):
        async with semaphore:
            ...
            kwargs = {k: v for k, v in request.items() if k not in ['messages', 'model']}
        ...
        try:
            ...
            for i, result in enumerate(batch_results):
                if isinstance(result, Exception):
                    ...
                    print(f"Request {i} result: {result['choices'][0]['message']['content']}")
        except Exception as e:
